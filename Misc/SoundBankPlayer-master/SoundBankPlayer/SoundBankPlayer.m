...
typedef struct
{
	double pitch;           // pitch of the note in the sound sample
...
typedef struct
{
	double pitch;      // pitch of the note
	int bufferIndex;  // which buffer is assigned to this note (-1 = none)
	double panning;    // < 0 is left, 0 is center, > 0 is right
}
Note;
...
- (void)registerAudioSessionCategory
{
    ...
    if (![session setCategory:AVAudioSessionModeDefault
                  withOptions:AVAudioSessionCategoryOptionDefaultToSpeaker
                        error:&setCategoryError]) {
        // handle error
        NSLog(@"HERE: %@", setCategoryError);
    }
}
...
- (void)noteOn:(int)midiNoteNumber gain:(double)gain
{
	[self queueNote:midiNoteNumber gain:gain];
	[self playQueuedNotes];
}

- (void)queueNote:(int)midiNoteNumber gain:(double)gain
{
	if (!_initialized)
	{
